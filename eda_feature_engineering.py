...
# Converted from 02_feature-engineering_testfile.ipynb
...
def main():
    ...
    for name, df in merged_data.items():
        print(f"\n--- EDA for {name} ---")
        print(df[['Return', 'MA_5', 'Vol_5', 'Return_lag1', 'GPRD', 'geo_keyword_hits', 'sentiment',
                  "Return_lag10"]].describe())
        # Correlations
        print("Correlation matrix:\n", df[
            ['Return', 'MA_5', 'Vol_5', 'Return_lag1', 'GPRD', 'geo_keyword_hits', 'sentiment', "Return_lag10"]].corr())
        # Distribution plots
    ...
    for name, df in merged_data.items():
        ...
        for name, df in merged_data.items():
            ...
            # Print mean volatility pre/post-event for quick check
